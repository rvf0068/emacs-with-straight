...
When pressing =RET=, in dired, files will be opened with =xdg-open=. If one wants to open in Emacs, use =C-u RET=. The setting for =process-connection-type= was needed, see https://askubuntu.com/a/824123/8975.

#+begin_src emacs-lisp
(use-package dired-open
  :straight t
  :custom
  (dired-open-functions '(dired-open-xdg))
